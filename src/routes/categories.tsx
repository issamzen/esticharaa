...
function CategoriesPage() {
  ...
  const [dbRows, setDbRows] = useState<DbCategory[] | null>(null);
  ...
  const visible: VisibleCategory[] = dbRows
    ? dbRows.map((row) => ({
        slug: row.slug,
        name: locale === "fr" && row.name_fr ? row.name_fr : locale === "en" && row.name_en ? row.name_en : row.name_ar,
        description: locale === "fr" ? row.description_fr : locale === "en" ? row.description_en : row.description_ar,
        icon: row.icon || "Sparkles",
        questions: Number(row.question_count) || 0,
        answers: Number(row.answer_count) || 0,
        experts: Number(row.expert_count) || 0,
        minimum: row.min_reward_tokens || 0,
      }))
    : fallbackCategories.map((base) => {
        const category = localizeCategory(base, locale);
        return { slug:category.slug,name:category.name,description:"",icon:category.icon,questions:category.questions,answers:category.answers,experts:category.experts,minimum:0 };
      });
